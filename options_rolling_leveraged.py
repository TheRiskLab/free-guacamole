...
weights=[(1.0, 0),(.95, .05), (0.9,0.1), 
         ...
         (0.7,0.3)]   ###1.0 is just to make sure the numbers are right and the same as the unhedged script

def download_data():
    ndx = yf.download('^NDX', start='1995-10-01', end='2002-10-01', auto_adjust=True)
    daily = ndx['Close'].pct_change().dropna()   ###try dropping the na part - wasn't in first script, same results? shouldn't matter by that reasoning so hell with it.
...
def rolling_annual_returns(monthly):
    ...
    if isinstance(r, pd.DataFrame):   ##just to fix a bug in the first run of the code, idk if it's necessary
        r = r.iloc[:,0]
    return r.squeeze()  #this makes sure it's a series, not a df
...
def calculate_hedged_returns(rolling,weights):   ###I explain the math and assumptions of tihs function in the read_me and my substack
    ...
    for ws,wo in weights:
        ...
        for ret in rolling:
            ...
            out.append((ws*mult + payoff)-1)   ###the issue here is that I'm mixing the value of the option with the return of the stock. 
        hedged[(ws,wo)] = out
...
#df.to_csv("options_hedged_rolling_returns.csv",index=False)

### Count positive vs negative for each hedge combo
###this is an interesting thingie - added after, not in my post.
